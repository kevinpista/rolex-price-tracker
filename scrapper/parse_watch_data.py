...
for ref_num in scrap_list:
    ...
    with open(f'{ref_num}.html') as html_file:
        ...
        for listing in soup.find_all('div', class_='media-flex-body d-flex flex-column justify-content-between'):
            ...
            # Box_and_Papers will be True by default as those are the only watches we'll filter for on scrapped website
            # Seller_location will be United States by default as those are the only watches we'll filter for on scrapped website
